...
import ssd1306  # Import the OLED library
...
i2c = I2C(-1, Pin(5), Pin(4))  # SDA and SCL pins
...
def wifi_connect():
    # Load login data from a different file for safety reasons
    ssid = secrets['ssid']
    password = secrets['pw']
...
def cet_time():
    year = time.localtime()[0]  # get the current year
    HHMarch = time.mktime((year, 3, (31 - (int(5 * year / 4 + 4)) % 7), 1, 0, 0, 0, 0, 0))  # Time of March change to CEST
    HHOctober = time.mktime((year, 10, (31 - (int(5 * year / 4 + 1)) % 7), 1, 0, 0, 0, 0, 0))  # Time of October change to CET
    now = time.time()
    if now < HHMarch:  # we are before the last Sunday of March
        cet = time.localtime(now + 3600)  # CET: UTC+1H
    elif now < HHOctober:  # we are before the last Sunday of October
        cet = time.localtime(now + 7200)  # CEST: UTC+2H
        print("we are before the last Sunday of October")
    else:  # we are after the last Sunday of October
        cet = time.localtime(now + 3600)  # CET: UTC+1H
        print("we are after the last Sunday of October")
    return cet
...
# import door_operations  # script that you want to execute at a certain moment

# Todo: create crontab-like structure with: minute / hour / day of the month / month / day of the week / command
# Todo: check what happens when scheduled tasks overlap (uasyncio)

def schedule(t):
    if t[tm_hour] == 17 and t[tm_min] == 45 and t[tm_sec] == 0:  # Define seconds, or it will run every second...
        print("Executing doorOperations")
        # door_operations.closeDoor()

    if t[tm_hour] == 07 and t[tm_min] == 45 and t[tm_sec] == 0:  # Define seconds, or it will run every second...
        print("Executing doorOperations")
        # door_operations.openDoor()

    # Sync the clock every day
...
def main():
    ...
    while True:
        ...
        if o_sec != t[5]:
            ...
            led.on()
            schedule(t)
            # Print the current time on the OLED screen
            oled.fill(0)  # Clear the screen
            oled.text("Time:", 0, 0)
            oled.text("{:02d}:{:02d}:{:02d}".format(t[tm_hour], t[tm_min], t[tm_sec]), 0, 20)
            oled.show()
            led.off()
